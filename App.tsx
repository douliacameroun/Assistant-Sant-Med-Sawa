            ...
            <motion.div key="chat" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex flex-col lg:flex-row h-full">
              ...
              <section className="flex-1 flex flex-col h-full bg-slate-50/10 backdrop-blur-sm relative">
                ...
                <div className="p-4 md:p-8 shrink-0 bg-white/90 backdrop-blur-2xl border-t border-slate-100 print:hidden">
                  <AnimatePresence>
                    {attachedFile && (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="mb-4 flex items-center gap-2 bg-[#F0F9FF] p-2 rounded-xl border border-blue-100">
                        <div className="bg-blue-500 text-white p-2 rounded-lg"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/></svg></div>
                        <span className="text-xs font-bold text-[#1B3B66] truncate max-w-[200px]">{attachedFile.name}</span>
                        <button onClick={() => setAttachedFile(null)} className="ml-auto text-slate-400 hover:text-red-500"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                      </motion.div>
                    )}
                  </AnimatePresence>
              ...
              <div className="max-w-6xl mx-auto space-y-6 print:space-y-8">
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                     ...
                     <div className="space-y-4">
                       {auditData?.identifiedPains.map((pain, i) => (
                         <div key={i} className="flex items-start gap-4 p-4 bg-red-50/50 rounded-2xl border border-red-100/50">
                           ...
                           <span className="font-bold text-[#1B3B66] text-sm">{pain}</span>
                         </div>
                       ))}
                     </div>
                   </div>
                   ...
                   <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 print:shadow-none print:border print:rounded-2xl">
                     <h2 className="text-[#C07D38] text-[10px] md:text-xs font-black uppercase mb-6 tracking-widest flex items-center gap-2">
                       <span className="w-1.5 h-1.5 bg-[#C07D38] rounded-full" />
                       Packs de Transformation Recommandés
                     </h2>
                     <div className="space-y-4">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
                     ...
                     {auditData?.transformationPlan.map((step, i) => (
                       <div key={i} className="relative z-10 flex flex-col items-center text-center space-y-4">
                         <div className="w-14 h-14 rounded-2xl bg-[#1B3B66] text-white flex items-center justify-center font-black text-xl shadow-xl ring-8 ring-white">
                           {i + 1}
                         </div>
                         <div className="bg-white px-4">
                           <h3 className="font-black text-[#1B3B66] text-sm uppercase mb-2 leading-tight">Phase {i+1}</h3>
                           <p className="text-slate-500 text-xs font-medium leading-relaxed">{step}</p>
                         </div>
                       </div>
                     ))}
                   </div>
